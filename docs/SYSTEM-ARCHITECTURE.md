...
Step-by-step install guides still live under [`docs/`](README.md) (01–05). This page is the **holistic map**.
...
flowchart LR
  ...
  subgraph Cloud["VPS"]
    WG -->|"UDP tunnel<br/>10.8.0.0/24"| WGS["WireGuard server<br/>10.8.0.1"]
...
| **WireGuard** (`wireguard`, `wireguard-tools`) | VPN | Private `10.8.0.0/24` network; server = `10.8.0.1` |
...
| **`scripts/setup-server.sh`** | Our installer | One-time: packages, WG, MediaMTX, firewall, creates `add-site.sh` |
| **`/root/add-site.sh`** | Our helper | Per site: WG peer, client `.conf`, MediaMTX paths, QR |
...
| Path | Contents |
|---|---|
| `/etc/wireguard/wg0.conf` | Server + all site peers |
| `/etc/wireguard/keys/` | Key material (secret) |
| `/etc/wireguard/allocated-ips.txt` | Which `10.8.0.N` is which site |
| `/root/site-configs/*.conf` | Per-box WireGuard configs |
| `/usr/local/etc/mediamtx.yml` | MediaMTX + camera paths |
| `/usr/local/bin/mediamtx` | Binary |

Docs: [04 — VPS setup](04-vps-server-setup.md) · [Server technical](technical/SERVER.md)
...
| Setting | Value | Why |
|---|---|---|
| WireGuard **Allowed IPs** | `10.8.0.0/24` (**not** `0.0.0.0/0`) | Split tunnel — keep LAN/DVR reachable |
...
| Camera | Main | Sub (used by MediaMTX `add-site` / grid) |
|---|---|---|
| 1 | 101 | **102** |
| 2 | 201 | **202** |
| 3 | 301 | **302** |
...
`add-site.sh` appends paths like:
...
| `rtspTransport: tcp` (global) | Required — relay is one TCP pipe |
| Browser URL | `http://<VPS-public-ip>:8889/site001_ch1` |
...
| DVR RTSP | `:554` |
| Box relay | `:8554` |
| Browser | `http://<public-ip>:8889/<site>_chN` |
...
flowchart TD
  ...
  D --> E["5. add-site.sh<br/>peer + QR + MediaMTX paths"]
  E --> F["6. WireGuard on box<br/>AllowedIPs 10.8.0.0/24, keepalive 25"]
  F --> G["7. Truvend Cam RTSP relay<br/>SERVER: LISTENING"]
  G --> H["8. Prove<br/>wg show → ping → ffprobe → browser"]
```

Detailed pages: [01](01-local-live-view.md) → [04](04-vps-server-setup.md) → [05](05-wireguard-on-box.md) → [02](02-rtsp-relay.md).
...
| 4 | VPS | `wg show` | Server sees peer |
...
| 9 | Laptop anywhere | `http://<public-ip>:8889/site001_ch1` | MediaMTX → browser |
...
| VPS WG + MediaMTX + add-site | Changing live-view UX |
...
| Installer script | [../scripts/setup-server.sh](../scripts/setup-server.sh) |
